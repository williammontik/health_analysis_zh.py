...
def build_summary_prompt(age, gender, country, concern, notes, metrics):
    metrics_summary = ", ".join(
        [f"{label} ({value}% )"
         ...
         for label, value in zip(block["labels"], block["values"])])[:9]
    return (
        ...
        "5. **整合数据**：每段话中都必须自然地融入至少一个具体的百分比数据。\n\n"
...
def build_suggestions_prompt(age, gender, country, concern, notes):
    return (
        f"为一位来自 {country}、{age} 岁、关注“{concern}”的{gender}，列出10条具体而温和的生活方式改善建议。\n"
        "⚠️ **严格指令**：请勿在首行添加任何寒暄（例如“当然可以”），直接以列表形式给出建议。\n"
...
def health_analyze():
    try:
        ...
        chart_prompt = (
            ...
            "每个类别必须以 '###' 开头，并包含 3 个真实世界指标，格式如 '指标名称: 68%'。所有百分比在 25% 到 90% 之间。仅返回 3 个区块，不要其他文字。"
        )
        ...
        creative_prompt = build_suggestions_prompt(age, user_info['gender'], user_info['country'], user_info['condition'], user_info['notes'])
        creative = get_openai_response(creative_prompt, temp=0.85)

        # --- Post-process to enforce formatting ---
        summary = re.sub(
            r'^(在分析[^。]+时)',
            lambda m: m.group(1).replace("在分析", "在相似群体中的"),
            summary
        )
        ...
        """
        email_subject = f"{content_lang.get('email_subject')} - {user_info.get('name', '')}"
